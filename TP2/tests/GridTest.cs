...
namespace Tests
{
    public class GridTest
    {
        ...
        [Fact]
        public void Grid_LoadFromCharArray()
        {
            char[,] charGrid = new char[4, 3]
            {
                { 'X', 'O', 'X' },
                { 'X', 'O', 'X' },
                { ' ', 'O', ' ' },
            ...
            Grid grid = new Grid(charGrid);

            Assert.Equal(4, grid.Rows);
            Assert.Equal(3, grid.Columns);

            Assert.Equal(GridValue.X, grid.GetCellValue(new Position(0, 0)));
            Assert.Equal(GridValue.O, grid.GetCellValue(new Position(0, 1)));
            Assert.Equal(GridValue.Empty, grid.GetCellValue(new Position(2, 0)));
        }

        [Fact]
        public void Grid_ExportToCharArray()
        {
            Grid grid = new Grid(3, 3);
            grid.SetCellValue(0, 0, GridValue.X);
            grid.SetCellValue(1, 0, GridValue.O);
            ...
            Assert.Equal(charGrid, grid.ToCharArray());
        }
    }
}
